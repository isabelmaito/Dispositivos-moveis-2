...
type StorageMode = "local" | "sqlite" | "mongodb"
...
export default function App() {
    ...
    const [idParaDeletar, setIdParaDeletar] = useState<number | string | null>(
        null,
    )
    const [modalEdicaoVisivel, setModalEdicaoVisivel] = useState(false)
    const [idParaEditar, setIdParaEditar] = useState<number | string | null>(
        null,
    )
    ...
    // Inicializa o banco SQLite local (necessário para o modo "local")
    useEffect(() => {
        ;(async () => {
    ...
    // Carrega usuários conforme o modo de armazenamento selecionado
    const carregarUsuarios = useCallback(async () => {
        if (storageMode === "local") {
            ...
            try {
                ...
                console.log("Erro ao carregar usuários locais:", error)
            }
        } else {
            try {
                const lista = await apiListarUsuarios(storageMode as ApiDbType)
                setUsuarios(lista)
            } catch (error) {
                console.log("Erro ao carregar via API:", error)
                Alert.alert(
                    "Backend não encontrado",
                    `Não foi possível conectar em: ${API_URL}\n\n` +
                        "Inicie o backend:\n" +
                        "  cd backend\n" +
                        "  npm run dev\n\n" +
                        "Dispositivo físico? Defina EXPO_PUBLIC_API_URL=http://<IP-da-máquina>:3000 no .env",
                )
            }
        }
    }, [db, storageMode])

    // Dispara carregarUsuarios sempre que o modo ou o db local mudar
    useEffect(() => {
        carregarUsuarios()
    }, [carregarUsuarios])

    // Troca o modo de armazenamento e limpa a lista antes de recarregar
    const handleChangeModo = (modo: StorageMode) => {
        setStorageMode(modo)
        setUsuarios([])
    }
    ...
    const handleCepChange = async (texto: string) => {
        ...
        if (apenasNumeros.length === 8) {
            setBuscandoCep(true)
            // Nos modos API usa o proxy do backend; em local usa viacep direto
            const endereco =
                storageMode !== "local"
                    ? ((await apiBuscarCep(apenasNumeros)) ??
        ...
        try {
            if (storageMode === "local") {
                if (!db) {
                    Alert.alert(
                        "Erro",
                        "Banco de dados ainda não inicializado. Aguarde um momento e tente novamente.",
                    )
                    return
                }
                ...
                await apiInserirUsuario(
                    storageMode as ApiDbType,
                    nome,
                    email,
                    cep,
                    logradouro,
            ...
            Alert.alert(
                isRede ? "Backend não encontrado" : "Erro ao cadastrar",
                isRede
                    ? `Não foi possível conectar em: ${API_URL}\n\nInicie o backend:\n  cd backend\n  npm run dev`
                    : error instanceof Error
                      ? error.message
    ...
    const handleDeletar = (id: number | string) => {
        setIdParaDeletar(id)
    ...
    const confirmarExclusao = async () => {
        ...
        try {
            if (storageMode === "local") {
                if (!db) return
                await deletaUsuario(db, idParaDeletar as number)
            } else {
                await apiDeletarUsuario(storageMode as ApiDbType, idParaDeletar)
            }
        ...
        <View style={styles.card}>
            ...
            <View style={styles.cardInfo}>
                ...
                <Text style={styles.cardTexto}>
                    {item.LOGRADOURO_US}
                    {item.NUMERO_US ? `, ${item.NUMERO_US}` : ""}
                    {item.COMPLEMENTO_US ? ` (${item.COMPLEMENTO_US})` : ""}
                </Text>
                <Text style={styles.cardTexto}>
                    {item.BAIRRO_US ? `${item.BAIRRO_US}, ` : ""}
                    {item.CIDADE_US}
                    {item.ESTADO_US ? ` - ${item.ESTADO_US}` : ""} | CEP:{" "}
                    {item.CEP_US}
                </Text>
            </View>
            <View style={styles.cardButtons}>
                ...
                <TouchableOpacity
                    ...
                    onPress={() => handleDeletar(item.ID_US)}
                >
        ...
        >
                ...
                <View style={styles.modalOverlay}>
                    <View style={styles.modalBox}>
                        ...
                        <View style={styles.modalBotoes}>
                            ...
                            >
                                <Text style={styles.btnConfirmarExcluirTexto}>
                                    Excluir
                                </Text>
                            </TouchableOpacity>
                        </View>
            ...
            >
                <View style={styles.modalOverlay}>
                    <View style={styles.modalEditBox}>
                        <ScrollView>
                            <Text style={styles.modalEditTitulo}>
                                Editar Usuário
                            </Text>
                            ...
                            <View style={styles.modalEditBotoes}>
                                ...
                                >
                                    <Text style={styles.btnSalvarEdicaoTexto}>
                                        Salvar
                                    </Text>
                                </TouchableOpacity>
                            </View>
            ...
            <View style={styles.header}>
                ...
                >
                    ...
                    <Text style={styles.headerSub}>
                        Preencha os dados e pesquise pelo CEP
                    </Text>
                </View>
            </View>
            ...
            <View style={styles.modeBar}>
                ...
                >
                    {(
                        [
                            { key: "local", label: "📱 Local SQLite" },
                            { key: "sqlite", label: "🗄️ API – SQLite" },
                            { key: "mongodb", label: "🍃 API – MongoDB" },
                        ] as { key: StorageMode; label: string }[]
                    ).map(({ key, label }) => (
                        <TouchableOpacity
                            key={key}
                            style={[
                                styles.modeTab,
                                storageMode === key && styles.modeTabAtivo,
                            ]}
                            onPress={() => handleChangeModo(key)}
                        >
                            <Text
                                style={[
                                    styles.modeTabTexto,
                                    storageMode === key &&
                                        styles.modeTabTextoAtivo,
                                ]}
                            >
                                {label}
                            </Text>
                        ...
                        {logradouro !== "" && (
                            <View style={styles.enderecoBox}>
                                <Text style={styles.enderecoTitulo}>
                                    📍 Endereço encontrado (edite se necessário)
                                </Text>
